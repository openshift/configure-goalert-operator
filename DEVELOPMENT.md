...
Quick reference for developing the Configure Goalert.
...
```bash
# Clone repository
git clone https://github.com/openshift/configure-goalert-operator.git
cd configure-goalert-operator
...
go test ./pkg/goalert/...  # Test specific package
...
```bash
# Run specific test
ginkgo -focus="CreateService" ./controllers/goalertintegration/

# Run tests for one package
go test -v ./controllers/goalertintegration/
...
- **API Types**: `api/v1alpha1/` - CRD definitions (GoalertIntegration)
- **Controllers**: `controllers/goalertintegration/` - Reconciliation logic
- **Business Logic**: `pkg/goalert/` - GoAlert client, `pkg/kube/` - K8s resource builders
- **Tests**: `*_test.go` alongside source, `*_suite_test.go` for Ginkgo
- **Metrics**: `pkg/localmetrics/` - Prometheus metrics
- **Config**: `config/` - Constants and resource naming

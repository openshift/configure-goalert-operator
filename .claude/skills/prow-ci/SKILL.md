...
# Prow CI Analysis for Configure GoAlert Operator
...
```text
https://prow.ci.openshift.org/view/gs/test-platform-results/pr-logs/pull/openshift_configure_goalert_operator/<PR_NUMBER>/pull-ci-openshift-configure-goalert-operator-master-lint/<BUILD_ID>
```
...
**Job History**: https://prow.ci.openshift.org/?repo=openshift%2Fconfigure-goalert-operator
...
- **Pull request jobs**: `gs://test-platform-results/pr-logs/pull/openshift_configure_goalert_operator/[PR_NUMBER]/[JOB_NAME]/[JOB_ID]`
- **Periodic jobs**: `gs://test-platform-results/logs/[JOB_NAME]/[JOB_ID]`

**Viewing logs via web**:
```text
https://prow.ci.openshift.org/view/gs/test-platform-results/pr-logs/pull/openshift_configure_goalert_operator/[PR_NUMBER]/[JOB_NAME]/[JOB_ID]
```
...
- `pull-ci-openshift-configure-goalert-operator-master-e2e-binary-build-success` - E2E binary build verification
- `pull-ci-openshift-configure-goalert-operator-master-coverage` - Code coverage analysis (with Codecov)
- `pull-ci-openshift-configure-goalert-operator-master-lint` - Linting checks
- `pull-ci-openshift-configure-goalert-operator-master-test` - Unit tests
- `pull-ci-openshift-configure-goalert-operator-master-validate` - Validation checks

**Tekton Pipelines** (configured in `.tekton/`):
- `configure-goalert-operator-pull-request` - Main PR pipeline (docker build with OCI-TA)
- `configure-goalert-operator-e2e-pull-request` - E2E testing pipeline
- `configure-goalert-operator-pko-pull-request` - PKO (Package Operator) pipeline
...
```text
https://prow.ci.openshift.org/?repo=openshift%2Fconfigure-goalert-operator&type=presubmit
```
...
- Configuration: `ci-operator/config/openshift/configure-goalert-operator/openshift-configure-goalert-operator-master.yaml`
- Runs: lint, test, validate, coverage, e2e-binary-build
- Uses Codecov for coverage reporting (secret: `configure-goalert-operator-codecov-token`)
...
- Builds container images to Quay (configure-goalert-operator-tenant)
...
gh pr checks <PR_NUMBER> | grep "pull-ci-openshift-configure-goalert-operator"
...
# https://prow.ci.openshift.org/?repo=openshift%2Fconfigure-goalert-operator

# Or use platform-specific command:
# macOS: open "https://prow.ci.openshift.org/?repo=openshift%2Fconfigure-goalert-operator"
# Linux: xdg-open "https://prow.ci.openshift.org/?repo=openshift%2Fconfigure-goalert-operator"
# Windows: start "https://prow.ci.openshift.org/?repo=openshift%2Fconfigure-goalert-operator"

# View specific PR on Prow (replace <PR_NUMBER>)
# https://prow.ci.openshift.org/?repo=openshift%2Fconfigure-goalert-operator&type=presubmit&pull=<PR_NUMBER>
```
...
- Prow jobs: `pull-ci-openshift-configure-goalert-operator-master-*`
- Tekton jobs: Usually show as "Tekton" or pipeline names in PR checks
- Verify repo name format in Prow: `openshift_configure_goalert_operator` (underscore, not dash)
...
- Common issues:
  ...
  - Auth failures → Secret configuration in `configure-goalert-operator-tenant` namespace
- Local validation:
  ```bash
...
- Location: `ci-operator/config/openshift/configure-goalert-operator/openshift-configure-goalert-operator-master.yaml`
- Update process: Submit PR to openshift/release repository
- Auto-generated jobs in: `ci-operator/jobs/openshift/configure-goalert-operator/`

**Tekton Pipelines** (in this repo):
- Location: `.tekton/` directory
- Files:
  - `configure-goalert-operator-pull-request.yaml` - Main PR pipeline
  - `configure-goalert-operator-push.yaml` - Post-merge pipeline
  - `configure-goalert-operator-e2e-pull-request.yaml` - E2E testing
  - `configure-goalert-operator-pko-pull-request.yaml` - PKO validation
...
- Secret: `configure-goalert-operator-codecov-token` (stored in Prow)
- Generate coverage locally: `make coverage`
- Coverage runs on PRs and post-merge (`publish-coverage`)
- Dashboard: Check Codecov for configure-goalert-operator

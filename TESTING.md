...
Testing guidelines for the Configure Goalert.
...
```go
It("should create GoAlert service", func() {
    // Create GoalertIntegration resource
    gi := &v1alpha1.GoalertIntegration{...}
    Expect(k8sClient.Create(ctx, gi)).To(Succeed())
    ...
    Expect(configMap.Data).To(HaveKey("low_integration_id"))
})
```
...
Example hook configuration (not currently enabled in this repo):
```yaml
- id: go-test
  entry: make go-test
  files: '\.go$'
...
This hook is **not** enabled in the current pre-commit config (too slow for pre-commit).
